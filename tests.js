...
let videos=await anime.getVideoSources(eps[0].url);assert.equal(videos.length,1);assert.equal(videos[0].audioLang,'ja');assert.equal(anime.stream('https://example.com/a.mp4',{name:'Dub',kind:'dub'},'https://example.com/',[]).audioLang,'en');assert.equal(videos[0].container,'hls');assert.equal(videos[0].subtitles[0].url,'https://player.example/en.vtt');assert.equal(videos[0].headers.Referer,'https://player.example/');

...
function parseCatalogue(body){
  ...
  return unique(cards.map(function(n){var a=first(n,function(x){return x.tag==='a'&&has(x,'name');}),img=first(n,function(x){return x.tag==='img';});if(!a||!a.a.href)return null;var u=absolute(a.a.href,SITE+'/').replace(/\/ep-\d+(?:[?#].*)?$/,'');var result=item(u,content(a),img?absolute(img.a['data-src']||img.a.src,SITE+'/'):null);if(a.a['data-jp'])result.englishTitle=text(a.a['data-jp']);return result;}).filter(Boolean),function(x){return x.id;});
}
function animeIdentity(title){
 var s=text(title).toLowerCase().replace(/&/g,' and ');if(s.normalize)s=s.normalize('NFD').replace(/[\u0300-\u036f]/g,'');
 s=s.replace(/\b(?:the)\b/g,' ').replace(/\b(?:dubbed|dub|subbed|sub|dual audio|1080p|720p)\b/g,' ');
 s=s.replace(/\b([2-9])(?:nd|rd|th)\s+season\b/g,'season $1').replace(/\bfirst\s+season\b/g,'season 1').replace(/\bsecond\s+season\b/g,'season 2').replace(/\bthird\s+season\b/g,'season 3');
 s=s.replace(/\b(?:season\s*|s0?)(\d+)\b/g,' season $1 ').replace(/\b(iii|ii|iv|vi|v)\b/g,function(v){return ' season '+({ii:2,iii:3,iv:4,v:5,vi:6}[v])+' ';});
 // Unnumbered first seasons and explicitly labelled season 1 share identity.
 return s.replace(/\bseason\s+1\b/g,' ').replace(/[^a-z0-9]+/g,'');
}
function animeSeasonFamily(title){return animeIdentity(String(title).replace(/\b(?:season\s*\d+|\d+(?:nd|rd|th)\s+season|s\d+|part\s*\d+|ii|iii|iv|v|vi)\b/gi,' '));}
function rankAnimeResults(rows,query){
 var wanted=animeIdentity(query),family=animeSeasonFamily(query);if(!wanted)return rows;
 return rows.map(function(row,index){var titles=[row.title,row.englishTitle||''],exact=titles.some(function(t){return animeIdentity(t)===wanted;}),sameFamily=titles.some(function(t){return animeSeasonFamily(t)===family;});
  // The app strips season/part markers when accepting a search result. Never
  // hand it a conflicting entry from that same franchise as a fallback.
  if(sameFamily&&!exact)return null;
  if(exact&&animeIdentity(row.title)===wanted&&row.title!==query)row.englishTitle=query;
  return {row:row,rank:exact?0:1,index:index};
 }).filter(Boolean).sort(function(a,b){return a.rank-b.rank||a.index-b.index;}).map(function(x){return x.row;});
}
async function search(query,page){query=String(query||'');return rankAnimeResults(parseCatalogue(await request(SITE+'/filter?keyword='+encodeURIComponent(query)+'&page='+pageNum(page)+'&vrf='+(query?encodeURIComponent(vrf(query)):''))),query);}
